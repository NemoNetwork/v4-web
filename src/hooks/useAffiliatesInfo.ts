...
export const useAffiliateMetadata = (dydxAddress?: string) => {
  ...
  const fetchAffiliateMetadata = async () => {
    ...
    const metadataEndpoint = `${compositeClient.indexerClient.config.restEndpoint}/affiliates/metadata`;
    const totalVolumeEndpoint = `${compositeClient.indexerClient.config.restEndpoint}/affiliates/total_volume`;
